...
class BLEProtocol:
    ...
    async def write_ble(self, data: bytes, verbose: bool = False) -> bool:
        ...
        if not self.client_ble or not self.client_ble.is_connected:
            logging.warning("⚠️ BLE client not connected")
            return False
            ...
            try:
                ...
                for d in devices:
                    if d.name and d.name.startswith(self.ble_name):
                        logging.info(f"✅ Device found: {d.address} ({d.name})")
                        return d.address
                        ...
                        for char in service.characteristics:
                            if "notify" in char.properties:
                                try:
                                    ...
                                    logging.warning(f"[Notify Error] {char.uuid}: {e}")
    ...
    def stop(self):
        ...
        logging.info("🛑 Stopping BLE loop...")
        self.ble_stop = True
