...
class SetupReader:
    ...
    async def setup_region(self):
        await self.send_data(
            [0xA5, 0x5A, 0x00, 0x0A, 0x2C, 0x01, 0x3C, 0x00, 0x0D, 0x0A]
        )
    ...
    async def setup_command_mode(self):
        await self.send_data(
            [0xA5, 0x5A, 0x00, 0x0A, 0xA1, 0x05, 0x00, 0x00, 0x0D, 0x0A]
        )

    async def setup_tag_focus(self):
        await self.send_data(
            [0xA5, 0x5A, 0x00, 0x0A, 0x60, 0x00, 0x00, 0x00, 0x0D, 0x0A]
        )

    async def setup_fastid_1(self):
        await self.send_data(
            [0xA5, 0x5A, 0x00, 0x0A, 0x5C, 0x01, 0x00, 0x00, 0x0D, 0x0A]
        )
    ...
    async def setup_fast_inventory(self):
        await self.send_data(
            [0xA5, 0x5A, 0x00, 0x0A, 0x64, 0x01, 0x00, 0x00, 0x0D, 0x0A]
        )
    ...
    async def setup_rf_link(self):
        await self.send_data(
            [0xA5, 0x5A, 0x00, 0x0B, 0x52, 0x00, 0x01, 0x05, 0x00, 0x0D, 0x0A]
        )
